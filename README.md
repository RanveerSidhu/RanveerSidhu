...
**Backend**
`ASP.NET Core` `C#` `Web APIs` `Entity Framework Core` `Dapper` `SQL Server` `REST APIs`

**Frontend**
`React` `JavaScript` `TypeScript` `Vite` `Ant Design` `Tailwind CSS`

**DevOps & Cloud**
`Docker` `GitHub Actions` `Azure` `Railway`

**Architecture**
`Clean Architecture` `SOLID Principles` `Repository Pattern` `JWT Authentication` `RESTful API Design` `CI/CD`
